...
- **App Store Connect IAP** on app `6809059582`: subscription group + `com.mabel.app.pro.monthly` / `com.mabel.app.pro.yearly` with 30-day (1-month) free intro. CIO/ASC may add Subscription Offer Codes on that same record; Eng does not invent prices. In-app redeem is Plans → Have a code? (`offerCodeRedemption`, macOS 15+). See [app-store-iap.md](app-store-iap.md). No free MAS ship until a Mac binary completes purchase / restore / manage.
