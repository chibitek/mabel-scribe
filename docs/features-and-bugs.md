...
### F3.2. Transforms (user-invoked rewrite) — SHIPPED as Pro (local-first)
Email | Bullet points | Make shorter | Make clearer. Sidebar Transforms is fourth in the Pro catalog (Dictionary → Snippets → Style → Transforms → Scratchpad → Insights). Pro surface: StoreKit entitlement AND Stiki session. Free is locked with Activate Pro / Sign in with Stiki. User-invoked only — never auto-on during dictation. Signed-in Pro invoke writes `transforms.json` on this Mac (last source + result). Local Gemma reshape; never invents facts; never leaves this Mac. Distinct from Style (Formal|Casual|Very casual register), Polish (Off|Casual|Professional|Polite Gemma tone), Snippets, Dictionary, and Clipboard History. Cloud / team share is not shipped (stub fail-closed until Enforcer BOUND). Stiki session is not folder ACL.

### F3.3. Scratchpad (local notes) — SHIPPED as Pro (local-first)
Local-only dictations/notes pad. Sidebar Scratchpad is fifth in the Pro catalog (Dictionary → Snippets → Style → Transforms → Scratchpad → Insights). Pro surface: StoreKit entitlement AND Stiki session. Free is locked with Activate Pro / Sign in with Stiki. Signed-in Pro add/edit/clear writes `scratchpad.txt` on this Mac. Notes stay on this Mac. Distinct from Style (Formal|Casual|Very casual register), Transforms (Email|Bullet points|Make shorter|Make clearer), Polish (Off|Casual|Professional|Polite Gemma tone), Dictionary, Snippets, and Clipboard History. Not an MCP source or sink in v1. No Nexus/Mochii promote. Cloud / team share is not shipped (stub fail-closed until Enforcer BOUND). Stiki session is not folder ACL. No HIPAA/BAA. Sign-out locks Scratchpad.

### F3.4. Insights (local usage) — SHIPPED as Pro (local-first)
Local-only dictation counts, streaks, and time-saved style. Sidebar Insights is sixth and last in the Pro catalog (Dictionary → Snippets → Style → Transforms → Scratchpad → Insights). Pro surface: StoreKit entitlement AND Stiki session. Free is locked with Activate Pro / Sign in with Stiki. Signed-in Pro reads `stats.json` on this Mac. Counts stay on this Mac — never the words. Distinct from Scratchpad (notes), Dictionary (spelling), Snippets (trigger→expansion), Style (Formal|Casual|Very casual register), Transforms (Email|Bullet points|Make shorter|Make clearer), Polish (Off|Casual|Professional|Polite Gemma tone), and Clipboard History. Not an MCP source or sink in v1. No cloud sync. No team/company dashboards. No third-party analytics vendor. No Nexus/SIEM write. Cloud / team share is not shipped (stub fail-closed until Enforcer BOUND). Stiki session is not folder ACL. No HIPAA/BAA. Free dictate stays ungated and still records local counts. Sign-out locks Insights until Stiki again. BREAKS IF: without dual gate; cloud analytics; cross-device sync; Nexus write; Free dictate gated; HIPAA.
