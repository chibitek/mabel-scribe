...
A native **iOS** sibling — **Mabel** (`com.mabel.ios`, v0.1.0) — lives in [`MabelIOS/`](MabelIOS/README.md). Custom keyboard overlay dictation in any app. Mabel cat UI only (no Flow brand). Explicit Start, sticky listen with **Mabel is on**, swipe-up confirm, Polish chips (existing catalog; default Off; Pro+Stiki). Keyboard is not a silent spy. Ship order Keyboard → Polish → Dictionary → Scratchpad → Languages. New ASC listing later. Not Mochii, not Mac TestFlight on a phone, not Spatial. Mac 1.4.0 paths below are unchanged.
