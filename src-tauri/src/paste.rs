...
impl ClipboardBackup {
    ...
    fn restore(self, clipboard: &mut Clipboard) {
        ...
        // Restore is a Mabel write, not a user Copy. Mark it so history
        // does not re-ingest the restored payload (and so a Concealed
        // original that arboard can only round-trip as plain text is not
        // captured after dictation paste).
        mark_mabel_pasteboard_write();
    }
}

/// Declare nspasteboard.org Transient + AutoGenerated on the current
/// general pasteboard. No-op off macOS.
pub fn mark_mabel_pasteboard_write() {
    #[cfg(target_os = "macos")]
    crate::pasteboard_macos::mark_mabel_pasteboard_write();
}
...
mod tests {
    ...
    #[test]
    fn paste_source_marks_transient_and_autogenerated() {
        let src = include_str!("paste.rs");
        assert!(src.contains("mark_mabel_pasteboard_write"));
        let macos = include_str!("pasteboard_macos.rs");
        assert!(macos.contains("org.nspasteboard.TransientType"));
        assert!(macos.contains("org.nspasteboard.AutoGeneratedType"));
        assert!(macos.contains("setData"));
    }
}
