...
Personal trigger → expansion. Sidebar Snippets is second in the Pro catalog (Dictionary → Snippets → Style → Transforms → Scratchpad → Insights). Pro surface: StoreKit entitlement AND Stiki session. Free is locked with Activate Pro / Sign in with Stiki. Signed-in Pro add/remove writes `snippets.json` on this Mac. Spoken triggers expand during the dictation cleanup pass. Distinct from Dictionary (spelling), Polish (tone), and Clipboard History. Cloud / team share is not shipped (stub fail-closed until Enforcer BOUND). Stiki session is not folder ACL.
